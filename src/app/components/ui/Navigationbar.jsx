...
const Navigationbar = () => {
  ...
  return (
    <div className="relative" ref={container}>
      ...
      {/* Menu Overlay */}
      <div
        className={`menu-overlay fixed top-0 left-0 w-full h-full p-8 glass-background-blue-violet z-50  flex flex-col transition-transform duration-300 ease-in-out ${isMenuOpen ? "translate-x-0" : "translate-x-full"}`}
      >
          ...
          <div className="flex-grow flex flex-col space-y-6 text-left">
            {Menulinks.map((Menulink, index) => (
              <div key={index} className="menu-link-item-holder relative">
                <Link
                  href={Menulink.link}
                  className="text-lime-500 text-3xl md:text-5xl font-PoppinsMedium leading-none hover:text-white transition-colors duration-300"
